...
double get_x_value(int curr_rank, int nodes_cnt) {
        ...
        if (curr_rank == 0) {
                ...
                for (auto dest_rank = 1; dest_rank < nodes_cnt; dest_rank++) {
                        ...
                        std::cout << "[HOST - 0] square root param value for rank '" << dest_rank << "' has just been sent" << std::endl;
                }
        } else {
                MPI_Recv(&input_x_value, 1, MPI_DOUBLE, 0, x_value_tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                std::cout << "[NODE - " << curr_rank << "] square root param value has just been received: '" << input_x_value << std::endl;
        }
...
double calc_series_element(double x, int seq_number) {
        auto numerator = pow(-1, seq_number) * factorial(2*seq_number);
        auto denominator = (1 - 2 * seq_number) * pow(factorial(seq_number), 2) * pow(4, seq_number);
...
int get_target_seq_number(int curr_rank, int nodes_cnt, int* last_seq_number) {
        ...
        if (curr_rank == 0) {
                ...
                for (auto dest_rank = 1; dest_rank < curr_rank; dest_rank++) {
...
bool get_break_flag(int curr_rank, int nodes_cnt, double* series_elements_sum, double series_element_value) {
        ...
        if (curr_rank == 0) {
                ...
                if (series_element_value < accuracy) {
...
double calculate_square_root(int curr_rank, int nodes_cnt, double x) {
        ...
        if (curr_rank == 0) {
                std::cout << "[HOST - 0] Calculation finished. Square root of '" << x << "' is equal " << series_elements_sum << std::endl;
        }
